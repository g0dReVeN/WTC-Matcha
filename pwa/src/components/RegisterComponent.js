...
const RegisterComponent = (props) => {
    ...
    const handleStatus = (key1, value) => {
        setValues({ ...values, [key1]: value });
    };
    ...
    const registerUser = event => {
        ...
        if (values.username && values.firstname && values.lastname && values.email && values.password) {
            axios.post(process.env.REACT_APP_API_URL + '/register', userInfo)
                .then(res => {
                    ...
                    if (res.status === 201) {
                        ...
                        setValues({ color: 'red', resMsg: res.data.msg });
                }).catch(err => {
